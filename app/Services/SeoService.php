...
class SeoService
{
    ...
    {
        $this->siteName = config('app.name', 'TECHNAZAARA');
        $this->siteUrl = config('app.url', url('/'));
        $this->defaultImage = asset('icon.png');
        $this->twitterHandle = '@technazaara'; // Update with your Twitter handle
    ...
    public function generate(array $data = [], ?string $pageKey = null): array
    {
        ...
        $description = $data['description'] ?? 'Latest technology news, tutorials, and insights from TECHNAZAARA. Stay updated with the latest trends in programming, web development, AI, and more.';
        $keywords = $data['keywords'] ?? 'technology, programming, web development, tutorials, tech news, coding, software development, TECHNAZAARA';

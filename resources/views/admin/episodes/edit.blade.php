...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-6">
        ...
        <p class="text-gray-600 dark:!text-text-secondary mt-1" style="font-family: 'Poppins', sans-serif; font-weight: 400;">
            Update episode details for {{ $content->title }}
        </p>
    </div>

    <div class="bg-white dark:!bg-bg-card rounded-lg border border-gray-200 dark:!border-border-secondary p-6">
        <form action="{{ route('admin.episodes.update', [$content, $episode]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="episode_number" id="episode_number" value="{{ old('episode_number', $episode->episode_number) }}" required
                ...
                <div>
                    <label class="flex items-center gap-2 mt-6">
                        <input type="checkbox" name="is_published" id="is_published" value="1" {{ old('is_published', $episode->is_published) ? 'checked' : '' }}
            ...
            <div class="mt-6">
                ...
                <textarea name="description" id="description" rows="4"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white dark:!bg-bg-card dark:!border-border-primary dark:!text-white focus:ring-2 focus:ring-accent focus:border-transparent"
                          style="font-family: 'Poppins', sans-serif; font-weight: 400;">{{ old('description', $episode->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                <div>
                    ...
                    <input type="date" name="air_date" id="air_date" value="{{ old('air_date', $episode->air_date ? $episode->air_date->format('Y-m-d') : '') }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white dark:!bg-bg-card dark:!border-border-primary dark:!text-white focus:ring-2 focus:ring-accent focus:border-transparent"
                           style="font-family: 'Poppins', sans-serif; font-weight: 400;">
                    @error('air_date')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="number" name="duration" id="duration" value="{{ old('duration', $episode->duration) }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white dark:!bg-bg-card dark:!border-border-primary dark:!text-white focus:ring-2 focus:ring-accent focus:border-transparent"
                           style="font-family: 'Poppins', sans-serif; font-weight: 400;">
                    @error('duration')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $episode->sort_order) }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white dark:!bg-bg-card dark:!border-border-primary dark:!text-white focus:ring-2 focus:ring-accent focus:border-transparent"
                           style="font-family: 'Poppins', sans-serif; font-weight: 400;">
                    @error('sort_order')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            ...
            <div class="mt-6">
                ...
                <input type="text" name="thumbnail_path" id="thumbnail_path" value="{{ old('thumbnail_path', $episode->thumbnail_path) }}"
